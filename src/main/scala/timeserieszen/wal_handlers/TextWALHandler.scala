...
class TextWALHandler(file: java.io.File) extends WALHandler[Double] {
  ...
  def writer = io.resource[FileWriter,DataPoint[Double] => Task[Unit]](Task.delay { new FileWriter(file, true) })( (f:FileWriter) => Task.delay {f.close()} )( (f:FileWriter) => Task.now {(d:DataPoint[Double]) => Task.delay { f.write(Utils.datapointToString(d)) } })
